...
export const PracticeConfig: React.FC<PracticeConfigProps> = ({onStart}) => {
    ...
    return (
        <div className="max-w-xl mx-auto bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-5">
            <div className="text-center">
                <div className="w-16 h-16 mx-auto bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center text-2xl font-bold mb-4">
                    🎯
                </div>
            ...
            <div className="space-y-4">
                <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Lộ trình</label>
                    <div className="flex flex-wrap gap-2 mt-1.5">
                        {TRACKS.map(({id, label}) => (
                            <button
                                key={id}
                                type="button"
                                onClick={() => { setTrack(id); setTopicId('all'); }}
                                className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                                    track === id ? 'bg-blue-600 text-white border-blue-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'
                                }`}
                            >
                                {label}
                            </button>
                ...
                <div>
                    <label htmlFor="practice-topic" className="text-xs font-bold text-slate-500 uppercase tracking-wider">Chủ đề</label>
                    <select
                        id="practice-topic"
                        value={topicId}
                ...
                <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Dạng câu hỏi</label>
                    <div className="flex flex-wrap gap-2 mt-1.5">
                        {TYPE_OPTIONS.map(({id, label}) => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                                    questionType === id ? 'bg-slate-900 text-white border-slate-900' : 'border-slate-200 text-slate-600 hover:bg-slate-50'
                                }`}
                            >
                                {label}
                            </button>
                ...
                <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Độ khó</label>
                    <div className="flex flex-wrap gap-2 mt-1.5">
                        {DIFFICULTY_OPTIONS.map(({id, label}) => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                                    difficulty === id ? 'bg-indigo-600 text-white border-indigo-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'
                                }`}
                            >
                                {label}
                            </button>
                ...
                <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Số câu hỏi</label>
                    <div className="flex flex-wrap gap-2 mt-1.5">
                        {COUNT_OPTIONS.map((n) => (
                            <button
                                key={n}
                                ...
                                className={`px-4 py-2 rounded-xl text-sm font-semibold border transition-all ${
                                    questionCount === n ? 'bg-emerald-600 text-white border-emerald-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'
                                }`}
                            >
                                {n}
                            </button>
            ...
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600 text-center">
                {previewCount === 0 ? (
                    <span className="text-rose-600 font-medium">Không có câu hỏi phù hợp.</span>
            ...
            <button
                ...
                onClick={handleStart}
                className="w-full py-3 font-bold text-white transition-all bg-blue-600 rounded-xl hover:bg-blue-700 shadow-md text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
