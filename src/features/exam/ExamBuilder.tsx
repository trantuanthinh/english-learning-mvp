...
export const ExamBuilder: React.FC<ExamBuilderProps> = ({onGenerate}) => {
    ...
    return (
        <div className="max-w-2xl mx-auto bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
            <div className="text-center">
                <div className="w-16 h-16 mx-auto bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-2xl font-bold mb-4">
                    📝
                </div>
                <h2 className="text-xl font-bold text-slate-900">Tạo bài thi tùy chỉnh</h2>
                <p className="text-xs sm:text-sm text-slate-500 mt-1">Cấu hình chủ đề, độ khó và dạng câu hỏi.</p>
            </div>

            {/* Topics */}
            <section className="space-y-3">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Chủ đề</h3>
            ...
            {/* Question Types */}
            <section className="space-y-3">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Dạng câu hỏi</h3>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input
                        type="checkbox"
                        checked={allTypes}
            ...
            {/* Difficulty */}
            <section className="space-y-3">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Độ khó</h3>
            ...
            {/* Question Count */}
            <section className="space-y-3">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Số câu hỏi</h3>
                <div className="flex flex-wrap gap-2">
                    {COUNT_OPTIONS.map((n) => (
                        <button
                            key={n}
            ...
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600">
                {previewPool.length === 0 ? (
                    <span className="text-rose-600 font-medium">Không có câu hỏi phù hợp với bộ lọc hiện tại.</span>
                ) : previewPool.length < questionCount ? (
                    <span>
                        Bạn yêu cầu <strong>{questionCount}</strong> câu, nhưng chỉ có{' '}
                        <strong>{previewPool.length}</strong> câu phù hợp. Bài thi sẽ gồm {previewPool.length} câu.
                    </span>
                ) : (
                    <span>
            ...
            <button
                ...
                className="w-full py-3.5 font-bold text-white transition-all shadow-md rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
